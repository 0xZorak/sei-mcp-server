...
app.post("/messages", (req: Request, res: Response) => {
  ...
  // Skip sessionId check for the /evm/tx/verify endpoint
  if (req.path !== "/evm/tx/verify") {
    if (!sessionId) {
    ...
    // Handle the /evm/tx/verify endpoint without sessionId check
    console.error(`Handling message for /evm/tx/verify`);
    // Add your handling logic for this endpoint here
  }
});

// Import the transaction service
import { getTransaction } from "../core/services/transactions.js"; // adjust path if needed

app.post("/evm/tx/verify", async (req: Request, res: Response) => {
  try {
    const { txHash, chainId, to, from, value } = req.body;
    if (!txHash || !chainId || !to || !from || !value) {
      return res
        .status(400)
        .json({ success: false, error: "Missing required fields" });
    }

    const network = chainId === 1329 ? "sei" : chainId.toString();
    const txDetails = await getTransaction(txHash);


    if (!txDetails || !txDetails.from || !txDetails.to) {
      return res
        .status(400)
        .json({ success: false, error: "Transaction not found" });
    }

    console.error("Comparing fields:");
    console.error("Expected from:", from, "Actual from:", txDetails.from);
    console.error("Expected to:", to, "Actual to:", txDetails.to);
    console.error("Expected value:", value, "Actual value:", txDetails.value);

    if (
      txDetails.from.toLowerCase() === from.toLowerCase() &&
      txDetails.to.toLowerCase() === to.toLowerCase() &&
      BigInt(txDetails.value) === BigInt(value)
    ) {
      return res.json({ success: true });
    } else {
      return res.status(400).json({ success: false, error: "Transaction fields do not match" });
    }
  } catch (error) {
